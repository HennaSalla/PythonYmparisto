...
class Code128B():
    ...
    def __init__(self, text: str, variant : str = 'Common') -> None:
        """Check if text contains only valid characters for code128B barcode.

        Args:
            text (str): A text string to be converted into a barcode.
            variant (str, optional): Variant of code 128B. Valid values: Common, Uncommon and BarcodeSoft. Defaults to 'Common'.
        """
        self.text = text
    ...
    def checkValidityOfText(self) -> bool | None:
        ...
        if self.variant == 'Common':
            ...
            for index in range(textLength):
                character = self.text[index]
                characterValue = ord(character)
                if characterValue in self.validRangeAll or characterValue in self.validBarcodeSoft or characterValue == self.barcodeSoftSpecialChar:
                    isValid = True
                else:
                    errorMessage = 'Text string contains invalid characters ' + '(' + character + ')'
                    raise ValueError(errorMessage)
    ...
    def buildBarcode(self) -> str:
        ...
        # Katsotaan onko tekstissä pelkästään sallituja merkkejä
        if self.checkValidityOfText() == True:
            rawTextLenght = len(rawText)
            weightedSum = 0

            #Käydään merkkijono silmukassa läpi ja lasketaan varmuussumman arvot
            for index in range(rawTextLenght):
                ...
                if characterValue < 127:
                    value = characterValue - 32 # Tätä arvoa käytetään varmistussumman laskenassa
                ...
                #Kirjaimen painotetun arrvon lisääminen, huom indexi alkaa 0:sta kertoimet 1:stä
                weightedSum = weightedSum + (index + 1) * value
            
            #Alkumerkin sisältävän arvon lisääminen
            weightedSum = weightedSum + startValues[variant] - subtractValues[variant]

            #Loppullinen varmistussumma jakojäännös 103:lla jaettuna
            checkSum = weightedSum % 103

            # Generoidaan lopullinen viivakoodi alkumerkki + raakatesti + varmistusumma + loppumerkki
